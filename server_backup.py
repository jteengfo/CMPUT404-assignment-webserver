        ...
        if decoded_data_split[0] == 'GET':
            ...
            if os.path.exists(complete_path):
                ...
                # check if file 
                    ...
                    invoke_isfile(self, decoded_data_split, complete_path)
                # else its a dir
                else:
                    # initialize checks variables
                    # do a left strip to see diff between /deep/ or deep/
                    string_lstrip = decoded_data_split[1].lstrip('/')
                    count = string_lstrip.count('/')

                    # check if just a forward slash, add index.html
...
def decode_data(byte_data):
    ''' a function that decodes the data from bytes to string '''
    decoded_data = byte_data.decode("utf-8")
    return decoded_data

def invoke_error405(self):
    ''' a function that invokes the http response 405 '''
    data = "405 Method Not Allowed"
    self.request.sendall(bytearray("HTTP/1.1 405 Not FOUND\n\n" + data, "utf-8"))

def invoke_error404(self):
    ''' a function that invokes the http response 404 '''
...
def invoke_dir_no_slash(self, decoded_data_split, complete_path):
    ''' a function that invokes a 301 http response because no forward slash '''
...
def invoke_dir_yes_slash(self, complete_path):
    ''' a function that invokes a 200 ok http response that directs to its index.html '''
    # print("It is a dir with a slash.\n")
    # check if an index.html exists
    default_path = complete_path + 'index.html'
    if os.path.exists(default_path):
        # print("An index.html does exist")
        # default_path = Path(default_path)
        # relative_path = decoded_data_split[1].lstrip('/')
...
def invoke_only_slash(self, complete_path):
    ''' a function that redirects to the index.html in the root folder '''
...
def invoke_isfile(self, decoded_data_split, complete_path):
    ''' a function that invokes a 200 ok http response, accessing a file that does exist '''
